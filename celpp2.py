...
<<<<<<< HEAD
											...
											if i.endswith("lig.pdb"):
												...
												proteinPDB=prody.parsePDB(y)
												ourPDB=prody.parsePDB('lmcss_rec.pdb')
												prody.matchChains(proteinPDB, ourPDB)
												protein_sp = prody.superpose(ourPDB, proteinPDB, weights=None)
												protein_sp.select(ligand+'_ligand.pdb')
												sts=str("obrms -f "+i+' '+id+"_docked.sdf")
												os.system(sts)
												f=open('visual.txt', 'ab+')
												f.write(x+'	smina	'+y+'\n')
