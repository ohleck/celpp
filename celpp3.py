									...
									if (os.path.isdir(cur)==True):
										...
										for i in (input):
											...
											if i.endswith("lig.pdb"): 
												...
												ourPDB=prody.parsePDB('lmcss_rec.pdb')
												a,b, seqid, overlap= prody.matchChains(proteinPDB, ourPDB)[0]
												b, protein_sp = prody.superpose(b, a, weights=None)
												extract =b.select('resname '+ligand)
												prody.writePDB('extracted_ligand.pdb', extract)												
												sts=str("obrms -f "+i+' '+id+"_docked.sdf")
												#run obrms
										...
										for i in (input):
											...
											if i.endswith("lig.pdb"):
												...
												proteinPDB=prody.parsePDB(y)
												ourPDB=prody.parsePDB('lmcss_rec.pdb')
												a,proteinPDB, seqid, overlap= prody.matchChains(proteinPDB, ourPDB)[0]
												proteinPDB, protein_sp = prody.superpose(proteinPDB, a, weights=None)
												extract =proteinPDB.select('resname '+ligand)
												x=set(extract.getResnames())
												print x
												extract.numAtoms()
												prody.writePDB('extracted_ligand.pdb', extract, csets=None)
												sts=str("obrms -f "+i+' '+id+"_docked.sdf")
												os.system(sts)
												f=open('visual.txt', 'ab+')
												f.write(x+'	smina	'+y+'\n')
